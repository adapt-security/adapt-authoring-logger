...
class LoggerModule extends AbstractModule {
  ...
  */
  async initialise() {
    ...
    const enabledLevels = this.getConfig('enabledLevels');
    /**
  ...
  */
  log(level, id, ...args) {
    ...
    console.log(`${LoggerModule.getDateStamp(this.config)}${LoggerModule.colourise(level, config.colour)} ${LoggerModule.colourise(id, 'magenta')}`, ...args);
  }
}
