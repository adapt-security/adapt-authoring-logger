...
class LoggerModule extends AbstractModule {
  ...
  */
  async initialise() {
    ...
    this.enabledLevels = this.getConfig('enabledLevels');
    /**
  ...
  isLevelEnabled(level) { // note explicit disable takes preference
    return !this.enabledLevels.includes(`!${level}`) && this.enabledLevels.includes(level);
  }
  ...
  */
  log(level, id, ...args) {
    ...
    console.log(`${LoggerModule.getDateStamp(this.config)}${LoggerModule.colourise(level, colour)} ${LoggerModule.colourise(id, 'magenta')}`, ...args);
  }
}
