const { AbstractUtility } = require('adapt-authoring-core');
...
class LoggerUtility extends AbstractUtility {
  ...
  */
  ...
  initialise() {
    const missingConfig = !this.app.config; // should still be able to operate without config
    ...
    */
    this.config = {
      levels: {
        error: {
          enable: missingConfig || enabledLevels.includes('error'),
          colour: 'red'
        },
        warn: {
          enable: missingConfig || enabledLevels.includes('warn'),
          colour: 'yellow'
        },
        success: {
          enable: missingConfig || enabledLevels.includes('success'),
          colour: 'green'
        },
        info: {
          enable: missingConfig || enabledLevels.includes('info'),
          colour: 'cyan'
        },
        debug: {
          enable: missingConfig || enabledLevels.includes('debug'),
          colour: 'grey'
        }
      },
      timestamp: missingConfig || this.app.config.get('adapt-authoring-logger.showTimestamp')
    };
  }
  /**
